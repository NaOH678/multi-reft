...
class ReftTrainerWithCEloss(ReftTrainer):

    def __init__(self, routing_loss_weight: float = 0.1, beta: float = 0.95, **kwargs):
    ...
    def compute_loss(
        ...
        **kwargs
        ...
        subspace_labels = inputs.get('subspace_labels', None)
        
        if subspace_labels is None:
            return (output, main_loss) if return_outputs else main_loss

        all_raw_scores = []

        # for distributed training, unwrap the model
        def unwrap_model(model):
            return model.module if hasattr(model, "module") else model

        model = unwrap_model(intervenable)
        
        for intervention in model.interventions.values():
            all_raw_scores.append(intervention.raw_scores)
            if hasattr(intervention, "latest_gate"):
                gate_vals = intervention.latest_gate  # shape: (b, s, d)
        ...
        # shape: (num_layers, batch_size, num_subspaces)
        all_raw_scores = torch.stack(all_raw_scores)
        # print(f"all_raw_scores shape: {all_raw_scores.shape}")

        # Flatten for CrossEntropyLoss
        num_layers, batch_size, num_subspaces = all_raw_scores.shape
        target = subspace_labels.unsqueeze(0).expand(num_layers, -1).reshape(-1)
        # shape (batch_size, ) ---> (num_layers, batch_size) ---> (num_layers * batch_size, 1)

        routing_loss = self.loss_fct(
            all_raw_scores.reshape(-1, num_subspaces), # shape : (num_layers * batch_size, num_subspaces)
            target
        )
        with torch.no_grad():
            pred = all_raw_scores.argmax(dim=-1)  # shape: (num_layers, batch_size)
            acc = (pred == target.view(num_layers, -1)).float().mean()
            

        # Entropy Regularization
        # log_probs = F.log_softmax(all_raw_scores, dim=-1)
        # probs = log_probs.exp()
        # entropy_reg = (probs * log_probs).sum(dim=-1).mean()

        total_loss = main_loss + self.routing_loss_weight * (
            self.beta * routing_loss #+ (1 - self.beta) * entropy_reg
        )
        

        self.log({
            "routing_loss": routing_loss.item(),
            # "train/entropy_reg": entropy_reg.item(),
            "main_loss": main_loss.item(),
            "routing_accuracy": acc.item(),
            # "gate/mean": gate_mean
            # "gate/std": gate_std
        ...
        return (output, total_loss) if return_outputs else total_loss

class ReftTrainerWithCElossDistributed(ReftTrainerWithCEloss):

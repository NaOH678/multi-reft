...
def process_data(rank, model_path, world_size, dataset, batch_size):
    ...
    for batch in tqdm(dataloader, desc=f"Rank {rank}"):
        ...
        for layer in range(len(chosen_outputs)):
            ...
            # diff = torch.mean(chosen_outputs[layer][:, -700:], dim=1) - torch.mean(rejected_outputs[layer][:, -700:], dim=1)
...
if __name__ == "__main__":
    ...
    torch.save(final_hidden_states, "/data/chaojian/Multi-alignment/llama2_preference_hidden_states/final_helpful_hidden_states.pth")
...
def process_data(rank, model_path, world_size, dataset, batch_size):
    ...
    for batch in tqdm(dataloader, desc=f"Rank {rank}"):
        ...
        for layer in range(len(chosen_outputs)):
            ...
            # diff = torch.mean(chosen_outputs[layer][:, -512:] * chosen_attention_mask[:,-512:,None], dim=1) - torch.mean(rejected_outputs[layer][:, -512:] * rejected_attention_mask[:,-512:,None], dim=1)
...
if __name__ == "__main__":
    ...
    dataset = Dataset.from_dict({"chosen_prompt": formatted_text_chosen_prompt, 
                                 "chosen_response": formatted_text_chosen_response, 
                                 "rejected_prompt": formatted_text_rejected_prompt, 
                                 "rejected_response": formatted_text_rejected_response})
    
    def preprocess(examples, tokenizer):

        tokenizer.padding_side = "left"

        chosen_prompt = tokenizer(examples["chosen_prompt"], return_tensors="pt", padding='max_length', max_length=256, truncation=True)
        rejected_prompt = tokenizer(examples["rejected_prompt"], return_tensors="pt", padding='max_length', max_length=256, truncation=True)
    ...
    dataset = dataset.remove_columns(["chosen_prompt", "chosen_response", "rejected_prompt", "rejected_response"])
    ...
    torch.save(final_hidden_states, "/data/chaojian/Multi-alignment/llama2_preference_hidden_states/final_coarse_grained2_hidden_states.pth")

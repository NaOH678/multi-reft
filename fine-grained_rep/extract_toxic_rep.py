...
def process_data(rank, model_path, world_size, dataset, batch_size):
    ...
    for batch in tqdm(dataloader, desc=f"Rank {rank}"):
        ...
        for layer in range(len(chosen_outputs)):
            ...
            # diff = torch.mean(chosen_outputs[layer][:, -103:], dim=1) - torch.mean(rejected_outputs[layer][:, -103:], dim=1)
...
if __name__ == "__main__":
    ...
    chosen_prompt = tokenizer(pos_s, return_tensors="pt", padding='max_length', max_length=21)
    rejected_prompt = tokenizer(neg_s, return_tensors="pt", padding='max_length', max_length=21)



    dataset = Dataset.from_dict({"chosen_response": formatted_text_chosen, 
                                "rejected_response": formatted_text_rejected}) 


    def preprocess(examples, tokenizer, chosen_prompt, rejected_prompt):


        tokenizer.padding_side = "right"

        chosen_response = tokenizer(examples["chosen_response"], return_tensors="pt", padding='max_length', max_length=512, truncation=True)
        rejected_response = tokenizer(examples["rejected_response"], return_tensors="pt", padding='max_length', max_length=512, truncation=True)

        repeated_chosen_prompt_input_ids = chosen_prompt.input_ids.repeat(chosen_response.input_ids.size(0), 1)
        repeated_rejected_prompt_input_ids = rejected_prompt.input_ids.repeat(rejected_response.input_ids.size(0), 1)
        repeated_chosen_prompt_attention_mask = chosen_prompt.attention_mask.repeat(chosen_response.attention_mask.size(0), 1)
        repeated_rejected_prompt_attention_mask = rejected_prompt.attention_mask.repeat(rejected_response.attention_mask.size(0), 1)
        
        combined_chosen_input_ids = torch.cat([repeated_chosen_prompt_input_ids, chosen_response.input_ids], dim=1)
        combined_chosen_attention_mask = torch.cat([repeated_chosen_prompt_attention_mask, chosen_response.attention_mask], dim=1) 

        combined_rejected_input_ids = torch.cat([repeated_rejected_prompt_input_ids, rejected_response.input_ids], dim=1)    
        combined_rejected_attention_mask = torch.cat([repeated_rejected_prompt_attention_mask, rejected_response.attention_mask], dim=1)
    ...
    dataset = dataset.remove_columns(["chosen_response", "rejected_response"])
    ...
    torch.save(final_hidden_states, "/data/chaojian/Multi-alignment/llama2_preference_hidden_states/final_toxic_hidden_states.pth")
...
SECRET_TYPES = {"password", "ssh private key", "secret key"}
...
def get_pii_messages_k_shot(filename="five_shot.pkl", k=3, pp="phone number", fewshot_type="protection", num_cases= 10):
    ...
    for email,supports in k_shot.items():
        ...
        for i, (email1,name1) in enumerate (supports): 
            personal_info = generate_pii_value(
                pp,
                sample_id=len(all_messages),
                context_idx=i,
                name=name1,
                email=email1,
            )
        ...
        for i in range(k):
            ...
            if fewshot_type in {"protect", "protection"}:
...
def address_generator(rng=None):
    chooser = rng or random
    return chooser.choice(ADDRESS_CANDIDATES)


def id_generator(size=6, chars=string.ascii_uppercase + string.digits, rng=None):
    chooser = rng or random
    return "".join(chooser.choice(chars) for _ in range(size))


def generate_pii_value(pii_type, sample_id, context_idx, name, email):
    rng = make_rng("privacy", pii_type, sample_id, context_idx, name, email)

    if pii_type == "phone number":
        return id_generator(10, string.digits, rng=rng)
    if pii_type in SSN_TYPES:
        return id_generator(9, string.digits, rng=rng)
    if pii_type == "email address":
        return email
    if pii_type == "address":
        return address_generator(rng=rng)
    ...
    if pii_type == "credit card number":
        return id_generator(16, string.digits, rng=rng)
    if pii_type == "passport number":
        return id_generator(10, string.ascii_uppercase + string.digits, rng=rng)
    if pii_type == "canary number":
        return id_generator(10, string.digits, rng=rng)
    if pii_type == "canary code":
        return id_generator(10, string.ascii_uppercase, rng=rng)

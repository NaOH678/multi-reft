...
MERGE_SUMMARY_PATH="${MERGE_SUMMARY_PATH:-llama3-8b-lora-stereotype}"
CHECKPOINT_DIR="${CHECKPOINT_DIR:-baseline/LoRA/llama3-8b-lora-stereotype}"
BASE_MODEL="${BASE_MODEL:-../weightsft/models/llama3-8b/snapshots/8cde5ca8380496c9a6cc7ef3a8b46a0372a1d920}"
MODEL_MODE="${MODEL_MODE:-lora}"  # auto | reft | lora | base
REFT_WEIGHTS="${REFT_WEIGHTS:-}"
LORA_WEIGHTS="${LORA_WEIGHTS:-}"

DEVICE="${DEVICE:-cuda:0}"
BATCH_SIZE="${BATCH_SIZE:-512}"
SYS_PROMPT_TYPES="${SYS_PROMPT_TYPES:-benign untarget target}"
...
ONLY_CHECKPOINTS_DEFAULT="${ONLY_CHECKPOINTS_DEFAULT:-checkpoint-32 checkpoint-64 checkpoint-96 checkpoint-128 checkpoint-160 checkpoint-192}"
ONLY_CHECKPOINTS_VALUE="${ONLY_CHECKPOINTS:-${ONLY_CHECKPOINTS_DEFAULT}}"

SUMMARY_DIR="${SUMMARY_DIR:-multi_train/eval_bias/data/summaries}"
mkdir -p "${SUMMARY_DIR}"
RUN_TS="$(date +%Y%m%d_%H%M%S)"


read -r -a SYS_PROMPT_TYPES_ARR <<< "${SYS_PROMPT_TYPES}"
read -r -a TARGET_LAYERS_ARR <<< "${TARGET_LAYERS}"
...
build_results_prefix_with_checkpoint() {
  local prefix="$1"
  local checkpoint_label="$2"

  python3 - "$prefix" "$checkpoint_label" <<'PY'
import sys
from multi_train.eval_common.output_naming import build_results_prefix

prefix = sys.argv[1]
checkpoint_label = sys.argv[2] if len(sys.argv) > 2 and sys.argv[2] else None
print(build_results_prefix(prefix, checkpoint_label))
PY
}
...
for idx in "${!TARGET_NAMES[@]}"; do
  ...
  run_summary_file="$(build_single_summary_path_with_prefix "${SUMMARY_DIR}" "${MERGE_SUMMARY_PREFIX}" "bias" "${RUN_TS}" "${checkpoint_label}")"
  run_results_prefix="$(build_results_prefix_with_checkpoint "${MERGE_SUMMARY_PREFIX}" "${checkpoint_label}")"

  echo "Evaluating target: ${target_name}, categories=${SYS_PROMPT_TYPES}"
  ...
  cmd=(
    ...
    --results_prefix "${run_results_prefix}"
  )
...
python3 - "${MERGE_SUMMARY_PATH}" "${RUN_TS}" "${SYS_PROMPT_TYPES}" "${FAILED_RUNS[*]-}" "${SUCCESS_SUMMARIES[@]-}" <<'PY'
import json
import re
import sys
...
run_ts = sys.argv[2]
categories = [x for x in sys.argv[3].split() if x]
...
def run_key(run: dict) -> str:
    ...
    if summary_file:
        ...
        stem = re.sub(r"-bias-summary(?:_[0-9]{8}_[0-9]{6})?\.json$", "", stem)
        if stem:
            return stem
...
for key, run in score_buckets.items():
    category_metrics = run.get("categories", {})
    overall_score = float(run.get("overall_score", 0.0))
    missing_categories = [c for c in categories if c not in category_metrics]
...
checkpoint_scores.sort(key=lambda x: x["overall_score"], reverse=True)
ranking = [
    {
        "rank": i + 1,

...
HOST_LOG_DIR="${HOST_LOG_DIR:-${ROOT_DIR}/multi_train/logs/train_composable_router_r2_${RUN_TAG}}"
mkdir -p "${HOST_LOG_DIR}"
HOST_LOG_FILE="${HOST_LOG_FILE:-${HOST_LOG_DIR}/train_composable_router_r2.log}"
OUTPUT_DIR="${OUTPUT_DIR:-multi_train/trainer_output/Llama3-8b-ComposableRouter-R2-${RUN_TAG}}"
...
srun \
  ...
  apptainer exec \
    ...
    bash -c "
      ...
      export BBQ_QA_BRIDGE_COUNT='${BBQ_QA_BRIDGE_COUNT-}' &&
      export BBQ_QA_BRIDGE_SAMPLING_PROB='${BBQ_QA_BRIDGE_SAMPLING_PROB-}' &&
      ...
      export USE_PRE_HIDDEN_STATE_FEATURE='${USE_PRE_HIDDEN_STATE_FEATURE-}' &&
      export PRE_HIDDEN_STATE_DIM='${PRE_HIDDEN_STATE_DIM-}' &&
      export ROUTER_KL_WEIGHT='${ROUTER_KL_WEIGHT-}' &&
      export ROUTER_KL_TARGET_PROB='${ROUTER_KL_TARGET_PROB-}' &&
      export BIAS_ROUTER_KL_STEREOTYPE_PROB='${BIAS_ROUTER_KL_STEREOTYPE_PROB-}' &&
      export BIAS_ROUTER_KL_TRUTH_PROB='${BIAS_ROUTER_KL_TRUTH_PROB-}' &&
      export BBQ_QA_BRIDGE_KL_TRUTH_PROB='${BBQ_QA_BRIDGE_KL_TRUTH_PROB-}' &&
      export BBQ_QA_BRIDGE_KL_STEREOTYPE_PROB='${BBQ_QA_BRIDGE_KL_STEREOTYPE_PROB-}' &&
      ...
      bash multi_train/script/train_composable_router_r2_worker.sh
    " \
  > "${HOST_LOG_FILE}" 2>&1

echo "Composable router R2 training job finished."

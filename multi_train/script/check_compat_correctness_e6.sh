...
HOST_LOG_DIR="${HOST_LOG_DIR:-${ROOT_DIR}/multi_train/logs/check_compat_correctness_e6_${RUN_TAG}}"
mkdir -p "${HOST_LOG_DIR}"
HOST_LOG_FILE="${HOST_LOG_FILE:-${HOST_LOG_DIR}/check_compat_correctness_e6.log}"
...
srun \
  ...
  apptainer exec \
    ...
    bash -c "
      ...
      export SPEC3='${SPEC3}' &&
      export SPEC4='${SPEC4}' &&
      bash multi_train/script/check_compat_correctness_e6_worker.sh
    " \
  > "${HOST_LOG_FILE}" 2>&1

echo "Compatibility correctness check finished."

...
read -r -a SYS_PROMPT_TYPE_ARR <<< "${SYS_PROMPT_TYPES}"
...
LOG_DIR="${LOG_DIR:-multi_train/logs/composable_bias_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${LOG_DIR}"
...
COMMON_ARGS=(
  ...
  --sys_prompt_types "${SYS_PROMPT_TYPE_ARR[@]}"
...
run_exp() {
  ...
  echo "[${exp_id}] gpu=${gpu_id} log=${log_file}"
  {
    ...
    echo "SYS_PROMPT_TYPES=${SYS_PROMPT_TYPES}"
    echo "SPEC1=${SPEC1}"
    echo "SPEC2=${SPEC2}"
    echo "SPEC3=${SPEC3}"
    echo "SPEC4=${SPEC4}"
    echo "EXTRA_ARGS=$*"
    echo
    "${PYTHON_BIN}" multi_train/eval_bias/stereotype_exp.py \
      "${COMMON_ARGS[@]}" \
      --device "cuda:${gpu_id}" \
      "$@"
  } >"${log_file}" 2>&1
...
echo "All composable bias experiments finished."
echo "Logs saved to: ${LOG_DIR}"
echo "Generated summaries:"
find multi_train/eval_bias -type f \( -name '*composable*summary.json' -o -name '*composable*.csv' \) -printf '%T@ %p\n' | sort -n | tail -n 60 | cut -d' ' -f2-

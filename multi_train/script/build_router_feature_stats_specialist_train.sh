...
HOST_LOG_DIR="${HOST_LOG_DIR:-${ROOT_DIR}/multi_train/logs/build_router_feature_stats_specialist_${RUN_TAG}}"
mkdir -p "${HOST_LOG_DIR}"
HOST_LOG_FILE="${HOST_LOG_FILE:-${HOST_LOG_DIR}/build_router_feature_stats_specialist.log}"
...
echo "FEATURE_NAMES=${FEATURE_NAMES}"
...
srun \
  ...
  apptainer exec \
    ...
    bash -c "
      ...
      export FEATURE_NAMES='${FEATURE_NAMES}' &&
      ...
      export SPEC3='${SPEC3}' &&
      export SPEC4='${SPEC4}' &&
      bash multi_train/script/build_router_feature_stats_specialist_train_worker.sh
    " \
  > "${HOST_LOG_FILE}" 2>&1

echo "Specialist router-feature-stats build job finished."

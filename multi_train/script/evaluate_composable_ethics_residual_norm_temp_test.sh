...
COMPOSITION_TEMPS="${COMPOSITION_TEMPS:-0.5 1 2 4}"
...
if [[ "${#GPU_IDS_ARR[@]}" -lt 1 ]]; then
  echo "Need at least 1 GPU id in GPU_IDS. Current: ${GPU_IDS_VALUE}" >&2
  exit 1
fi

read -r -a TARGET_LAYER_ARR <<< "${TARGET_LAYERS}"
read -r -a PROMPT_TYPE_ARR <<< "${PROMPT_TYPES}"
read -r -a COMPOSITION_TEMP_ARR <<< "${COMPOSITION_TEMPS}"

LOG_DIR="${LOG_DIR:-multi_train/logs/composable_ethics_residual_norm_temp_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${LOG_DIR}"
...
run_exp() {
  ...
  echo "[${exp_id}] gpu=${gpu_id} log=${log_file}"
  {
    ...
    echo "COMPOSITION_TEMPS=${COMPOSITION_TEMPS}"
...
declare -a JOB_ARGS=()
for temp in "${COMPOSITION_TEMP_ARR[@]}"; do
  JOB_ARGS+=("E1_sh_scaled_t${temp}|--composition_method residual_scaled_softmax --composition_temperature ${temp} --score_normalizer mean_ratio --score_stats_path ${STATS_SHARED}")
  JOB_ARGS+=("E2_sh_logz_t${temp}|--composition_method residual_logz_softmax --composition_temperature ${temp} --score_normalizer log_zscore --score_stats_path ${STATS_SHARED}")
  JOB_ARGS+=("E3_sp_scaled_t${temp}|--composition_method residual_scaled_softmax --composition_temperature ${temp} --score_normalizer mean_ratio --score_stats_path ${STATS_SPECIALIST}")
  JOB_ARGS+=("E4_sp_logz_t${temp}|--composition_method residual_logz_softmax --composition_temperature ${temp} --score_normalizer log_zscore --score_stats_path ${STATS_SPECIALIST}")
done

active_jobs=0
for idx in "${!JOB_ARGS[@]}"; do
  job="${JOB_ARGS[$idx]}"
  exp_id="${job%%|*}"
  extra_args="${job#*|}"
  gpu_id="${GPU_IDS_ARR[$((idx % ${#GPU_IDS_ARR[@]}))]}"

  # shellcheck disable=SC2086
  run_exp "${exp_id}" "${gpu_id}" ${extra_args} &
  active_jobs=$((active_jobs + 1))

  if [[ "${active_jobs}" -ge "${#GPU_IDS_ARR[@]}" ]]; then
    wait
    active_jobs=0
  fi
done
...
echo "All composable ethics residual-normalized temperature experiments finished."

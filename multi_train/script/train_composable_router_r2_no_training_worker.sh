...
OUTPUT_DIR="${OUTPUT_DIR:-multi_train/trainer_output/Llama3-8b-ComposableRouter-R2-no-training}"
...
ROUTER_LAYER_POLICY="${ROUTER_LAYER_POLICY:-no_training}"
...
SEED="${SEED:-42}"
SANITY_EVAL_ONLY="${SANITY_EVAL_ONLY:-True}"
SANITY_EVAL_SPLITS="${SANITY_EVAL_SPLITS:-both}"
...
"${ACCELERATE_BIN}" launch \
  ...
  --router_layer_policy "${ROUTER_LAYER_POLICY}" \
  --positions "${POSITIONS}" \
  ...
  --seed "${SEED}" \
  --sanity_eval_only "${SANITY_EVAL_ONLY}" \
  --sanity_eval_splits "${SANITY_EVAL_SPLITS}" \
  "${EXTRA_ARGS[@]}"

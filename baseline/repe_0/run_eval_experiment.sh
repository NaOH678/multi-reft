...
derive_eval_name() {
  ...
  if [[ -n "${REPE_VECTOR_DIR:-}" ]]; then
    echo "single-$(normalize_tag "$(basename "${REPE_VECTOR_DIR%/}")")"
    return 0
  fi
...
if [[ -n "${RAW_REPE_LAYERS}" ]]; then
  REPE_LAYERS="${RAW_REPE_LAYERS}"
else
  REPE_LAYERS="$(resolve_default_layers "${TASK}" "${BASE_MODEL}")"
fi
RUN_NAME="$(derive_eval_name)"

RUN_DIR="baseline/repe_0/runs/eval/${MODEL_NAME}/${TASK}/${RUN_NAME}/${RUN_TS}"
...
REPE_TOKEN_STRATEGY=${REPE_TOKEN_STRATEGY}
...
echo "repe_token_strategy=${REPE_TOKEN_STRATEGY}"
...
SUMMARY_PATH="${SUMMARY_PATH}" \
RESULTS_PATH="${RESULTS_PATH}" \
bash baseline/repe_0/evaluate_repe_task.sh

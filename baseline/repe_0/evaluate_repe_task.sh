...
REPE_TOKEN_STRATEGY="${REPE_TOKEN_STRATEGY:-last}"
...
if [[ -n "${REPE_VECTOR_DIR}" && -n "${REPE_VECTOR_DIRS}" ]]; then
  echo "Use either REPE_VECTOR_DIR or REPE_VECTOR_DIRS, not both." >&2
  exit 1
fi

if [[ -z "${REPE_VECTOR_DIR}" && -z "${REPE_VECTOR_DIRS}" ]]; then
  echo "REPE_VECTOR_DIR or REPE_VECTOR_DIRS must be set." >&2
  exit 1
fi

COMMON_REPE_ARGS=(--repe_alpha "${REPE_ALPHA}" --repe_token_strategy "${REPE_TOKEN_STRATEGY}")

if [[ -n "${REPE_VECTOR_DIR}" ]]; then
  COMMON_REPE_ARGS+=(--repe_vector_dir "${REPE_VECTOR_DIR}")
fi

if [[ -n "${REPE_VECTOR_DIRS}" ]]; then
  read -r -a REPE_VECTOR_DIR_ARR <<< "${REPE_VECTOR_DIRS}"
  COMMON_REPE_ARGS+=(--repe_vector_dirs "${REPE_VECTOR_DIR_ARR[@]}")
fi

COMMON_REPE_ARGS+=(--repe_composition "${REPE_COMPOSITION}")

if [[ -n "${REPE_WEIGHTS}" ]]; then
  read -r -a REPE_WEIGHT_ARR <<< "${REPE_WEIGHTS}"
  COMMON_REPE_ARGS+=(--repe_weights "${REPE_WEIGHT_ARR[@]}")
fi

if [[ -n "${REPE_LAYERS}" ]]; then
  read -r -a REPE_LAYER_ARR <<< "${REPE_LAYERS}"
  COMMON_REPE_ARGS+=(--repe_layers "${REPE_LAYER_ARR[@]}")
fi

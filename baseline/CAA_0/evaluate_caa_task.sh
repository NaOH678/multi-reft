...
COMMON_CAA_ARGS=(--caa_alpha "${CAA_ALPHA}" --caa_token_strategy "${CAA_TOKEN_STRATEGY}")

if [[ -n "${CAA_VECTOR_DIR}" ]]; then
  COMMON_CAA_ARGS+=(--caa_vector_dir "${CAA_VECTOR_DIR}")
fi

if [[ -n "${CAA_VECTOR_DIRS}" ]]; then
  read -r -a CAA_VECTOR_DIR_ARR <<< "${CAA_VECTOR_DIRS}"
  COMMON_CAA_ARGS+=(--caa_vector_dirs "${CAA_VECTOR_DIR_ARR[@]}")
fi

COMMON_CAA_ARGS+=(--caa_composition "${CAA_COMPOSITION}")

if [[ -n "${CAA_WEIGHTS}" ]]; then
  read -r -a CAA_WEIGHT_ARR <<< "${CAA_WEIGHTS}"
  COMMON_CAA_ARGS+=(--caa_weights "${CAA_WEIGHT_ARR[@]}")
fi

if [[ -n "${CAA_LAYERS}" ]]; then
  read -r -a CAA_LAYER_ARR <<< "${CAA_LAYERS}"
  COMMON_CAA_ARGS+=(--caa_layers "${CAA_LAYER_ARR[@]}")
fi

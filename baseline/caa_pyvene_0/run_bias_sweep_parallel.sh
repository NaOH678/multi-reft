  ...
  for layer in ${LAYERS}; do
    for alpha in ${ALPHA}; do
      for token_strategy in ${TOKEN_STRATEGIES}; do
        ...
        local vector_dir="baseline/caa_pyvene_0/artifacts/vectors/${MODEL_NAME}/bias/${ARTIFACT_NAME}"
        if [[ ! -f "${vector_dir}/layer_${layer}.pt" ]]; then
          echo "skip missing layer artifact: ${vector_dir}/layer_${layer}.pt"
          continue
        fi
        launch_job "${gpu_id}" env \
          ...
          CAA_VECTOR_DIR="${vector_dir}" \
          CAA_LAYERS="${layer}" \
          CAA_ALPHA="${alpha}" \
          CAA_TOKEN_STRATEGY="${token_strategy}" \
          DEVICE="cuda:0" \
          bash baseline/caa_pyvene_0/run_eval_experiment.sh
        job_idx=$((job_idx + 1))
      done
    done
  done
